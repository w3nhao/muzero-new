...
import models
from mcts import MCTS, Node, SelfExploration
...
class DiagnoseModel:
    ...
    def get_virtual_trajectory_from_obs(self, observation, horizon, plot=True, to_explore=0):
        ...
        for i in range(horizon):
            action = SelfExploration.select_action(root, 0)
            ...
            root, mcts_info = MCTS(self.config).run(
                self.model, None, self.config.action_space, virtual_to_explore, True, root
            )
    ...
    def compare_virtual_with_real_trajectories(
        self, first_obs, simulation, horizon, plot=True
    ):
        """
        First, MuZero explores a simulation but uses its model instead of using the environment.
        Then, MuZero explores the optimal trajectory according precedent trajectory but performs it in the
        ...
        All information during the two trajectories are recorded and displayd.
        """
        ...
        for i, action in enumerate(virtual_trajectory_info.action_history):
            ...
            if action not in simulation.legal_actions():
                break  # Comment to keep exploreing after trajectory divergence
                action = SelfExploration.select_action(root, 0)
